...
pub fn default(html: &mut String) {
  ...
  re.captures_iter(&html.clone()).for_each(| capture: Captures | {
    ...
    .enumerate().for_each(| (i, blockquote): (usize, Blockquote) | {
      ...
      // Add text & cite to blockquote template
      let blockquote_html: String = "\n<blockquote>%text\n<cite>%cite</cite>\n%child</blockquote>\n\n"
                                      .replace("%text", &blockquote.text)
